...
class FcmService {
  ...
  static Future<void> initialize() async {
    try {
      // 1. Initialize Firebase App with short safety timeout
      try {
        await Firebase.initializeApp(
          options: DefaultFirebaseOptions.currentPlatform,
      ...
      // 2b. Initialize local notifications (Android channel + foreground fallback)
      await _initializeLocalNotifications();

      // 3. Request user permissions (iOS & Android 13+) safely
      try {
        ...
        developer.log('FCM Permission request error/timeout: $e', name: 'FCMService');
      }
  ...
  /// Subscribe to default topics with timeout guard
  static Future<void> _subscribeToDefaultTopics() async {
    try {
      await _messaging.subscribeToTopic('sti_delegates').timeout(const Duration(seconds: 3));
      await _messaging.subscribeToTopic('all_orders').timeout(const Duration(seconds: 3));
    } catch (e) {
  ...
  static Future<void> _initializeLocalNotifications() async {
    ...
    developer.log('Local notifications & Android channel initialized', name: 'FCMService');
  }
  ...
  static Future<void> syncTokenWithBackend(String token) async {
    try {
      ...
      // Subscribe to common notification topics safely with short timeout
      try {
        await _messaging.subscribeToTopic('sti_delegates').timeout(const Duration(seconds: 2));
        await _messaging.subscribeToTopic('all_orders').timeout(const Duration(seconds: 2));
        if (user?['region'] != null) {
          final cleanRegion = user!['region'].toString().replaceAll(' ', '_').toLowerCase();
          await _messaging.subscribeToTopic('region_$cleanRegion').timeout(const Duration(seconds: 2));
        }
      } catch (_) {}
  ...
  static void _setupMessageHandlers() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      InAppNotificationBanner.show(
        ...
        icon: type == 'monthly_objective' || type == 'objective'
            ? Icons.track_changes_rounded
            : (type == 'order_submitted' || type == 'order_created'
